...
export function MerchantSignage({ merchant }: Props) {
  return (
    <div className="mx-auto w-full max-w-3xl rounded-2xl border bg-white p-8 text-right print:max-w-none print:border-none print:p-0">
      ...
      <div className="space-y-5">
        {merchant.branches.map((branch) => (
          <section key={branch.id} className="rounded-xl border p-4">
            ...
            <ul className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              {branch.walletAccounts.length > 0 ? (
                branch.walletAccounts.map((wallet) => (
                  <li key={wallet.id} className="rounded-lg border bg-slate-50 p-3">
